...
import { formatDuration, formatPrice } from '@/helpers/utilities';
...
function getOrderDuration(order: Order, now: number): string {
	const created = order.createdAt ? new Date(order.createdAt).getTime() : 0;
	if (!created) return '0:00';
	if (order.status === OrderStatus.DELIVERED && order.updatedAt) {
...
function getOrderTotal(order: Order): { amount: number; currency: string } | null {
	...
	return (
		<div
			className={classNames(s['pk-column'], {
				...
				[s['pk-column--status-in-progress']]: status === OrderStatus.IN_PROGRESS,
			...
			<div className={s['pk-column__cards']}>
				{orders.map((order) => {
					...
					const isClickable = isReadyColumn 
						? hasRider && !!onClick 
						: !!onClick && !isHistory;
					const canGoBack = !!onGoBack && getPreviousStatus(order.status) !== null;
					...
					const cardContent = (
						<>
							...
							<div className={s['pk-card__duration']} aria-label={`Tiempo del pedido: ${getOrderDuration(order, now)}`}>
								<Clock className={s['pk-card__duration-icon']} size={18} aria-hidden />
							...
							<div className={s['pk-card__items']}>
								{order.items.map((item, idx) => (
									<div key={item.id || idx} className={s['pk-card__item']}>
										...
										<span className={s['pk-card__item-name']}>
											{item.name}
										</span>
										{item.price && (
											<span className={s['pk-card__item-unit-price']} title="Precio unitario">
							...
							{hasRider && !isHistory && (
								<div className={s['pk-card__rider']}>
									<Bike className={s['pk-card__rider-icon']} size={14} aria-hidden />
									Rider esperando
								</div>
							)}
					...
					return (
						<div
							...
							data-ready-with-rider={isReadyColumn && hasRider ? 'true' : undefined}
							className={classNames(s['pk-card'], {
								...
								[s['pk-card--has-rider']]: hasRider && !isReadyColumn && !isHistory,
							})}
						>
							{isClickable ? (
								<button
									...
									aria-label={isReadyColumn ? `Entregar orden ${order.externalId}` : `Avanzar orden ${order.externalId}`}
								>
									{cardContent}
								</button>
							) : (
								...
								<div className={s['pk-card__action-row']}>
									<div className={s['pk-card__action-buttons']}>
										...
										{onEdit && !isHistory && (
											...
											>
												<Pencil className={s['pk-card__edit-icon']} size={14} aria-hidden />
												Editar
											</button>
										)}
									</div>
									{isClickable && (
										<div className={s['pk-card__action']}>
											{isReadyColumn ? 'Click para entregar ✓' : 'Click para avanzar →'}
										</div>
									)}
								</div>
							)}
